...
<%
    ...
    String query = "UPDATE stok_bbm_detail set pemakaian_oli = "+baru+" WHERE witel='"+witel+"' AND tanggal='"+yyyymmdd+"' AND node='"+node+"';";
    boolean canProceed = con.executeQuery(query);
    if(canProceed)
        canProceed = con.executeQuery(bbm.calcJumlahPemakaianOli());
    
    
    con.activitLog(nik, "stok_bbm_detail", "Mengganti data pemakaian oli node "+node+" dari "+lama+" menjadi "+baru+" untuk laporan stok BBM bulan "+tanggal);
    con.closeConnection();
    
    out.print(new BasicJson().getBasicJson(true, "Data pemakaian oli untuk node "+node+" telah diperbarui").toString());
